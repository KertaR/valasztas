...
import { MapPin, ChevronDown, ChevronUp, Map, Users, UsersRound, PieChart } from 'lucide-react';
                ...
                <table className="w-full text-left border-collapse min-w-[600px] transition-colors">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800 transition-colors">
                        {data.map((county, idx) => {
                            ...
                            return (
                                <React.Fragment key={county.id || idx}>
                                    <tr
                                        onClick={() => toggleRow(county.id)}
                                        className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group cursor-pointer ${isExpanded ? 'bg-slate-50 dark:bg-slate-800/80 shadow-inner border-l-2 border-l-indigo-500' : 'border-l-2 border-l-transparent'}`}
                                    >
